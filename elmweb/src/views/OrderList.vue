    ...
    <ul class="order">
      <li v-for="item in orderUnpaidArr" :key="item.orderId">
        ...
        <ul class="order-detailet" v-show="item.isShowDetailet">
          ...
          <li>
            <i><strong style="font-weight:bold">订购日期</strong></i>
            <i><strong style="font-weight:bold">{{ item.orderDate }}</strong></i>
          </li>
        </ul>
      </li>
    </ul>
...
const init = () => {
  let ordersUrl = `OrdersController/listOrdersByUserId/${user.userId}`
  axios
    .get(ordersUrl)
    .then((response) => {
      ...
      for (let orders of result) {
        ...
        if (orders.orderState == 0) {
          orderUnpaidArr.value.push(orders);
        }
        if (orders.orderState == 1) {
          orderPaidArr.value.push(orders);
        }
      }
    })
    .catch((error) => {
